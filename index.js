...
PROMISE_POLLYFILL = () => {
  ...
  //   resolvedData;
  //   rejectedData;
  //   // thenFn; // for single then block
  //   resolveChain = []; // for chaining then block
  //   rejectChain = [];
  //   isResolved = false;
  //   isRejected = false;
  ...
  //   then(fn) {
  //     // this.thenFn = fn; // for single then block
  //     this.resolveChain.push(fn); // for chaining then block
  //     if (this.isResolved) {
  //       // for sychronous call
  ...
  //       this.resolveChain.reduce((acc, fn) => fn(acc), this.resolvedData);
  //     }
  //     return this; // for then chaining eg: then().then()
  //   }
  ...
  class MyPromise {
    ...
    constructor(executor) {
      const resolve = (value) => {
        ...
        // for chaining then block
        console.log("resolve => this.resolveChain => ", this.resolveChain);
        if (this.resolveChain.length) {
          this.resolveChain.reduce((acc, fn) => fn(acc), this.resolvedData);
        }
      };
    ...
    then(fn) {
      // this.thenFn = fn; // for single then block
      this.resolveChain.push(fn); // for chaining then block
      console.log("this.resolveChain", this.resolveChain);
      if (this.isResolved) {
        // for sychronous call
        // fn(this.resolvedData);
  ...
  //   .then((data) => data * 2)
  //   .then((data) => {
  //     console.log("second chain data => ", data);
  //     return data * 2;
  //   })
  //   .then((data) => {
  //     // console.log(data);
  //     // return "setTimeOut chain";
  //     console.log("Third chain data => ", data);
  //     return data;
  //   })
  //   .catch((err) => {
  //     return `${err} :(`;
  //   })
  //   .catch((errInfo) => {
  //     return `${errInfo} :: send to Finally`;
  //   })
  //   .finally((data) => console.log(data));
  // // .then((data) => {
  // //   setTimeout(() => console.log(`${data} 30`), 1000);
  // // });
  new MyPromise((resolve, reject) => {
    resolve(10);
  })
